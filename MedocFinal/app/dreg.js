...
const AddEmployee = ({navigation,route}) => {
  ...
  const getDetails = (type)=>{
    if(route.params){
       switch(type){
           ...
           case "docregisterno":
              return  route.params.docregisterno
           case "specilization":
              return  route.params.specilization    
           case "phonenumber":
              return  route.params.phonenumber
           case "hospitalname":
              return  route.params.hospitalname  
           case "hospitaladdress":
              return  route.params.hospitaladdress  
           case "email":
 ...
 function notifyMessage(msg) {
  ...
  const [docregisterno,setdocregisterno]=useState(getDetails("docregisterno"))
  const [specilization,setspecilization]=useState(getDetails("specilization"))
  const [hospitalname,sethospitalname]=useState(getDetails("hospitalname"))
  const [hospitaladdress,sethospitaladdress]=useState(getDetails("hospitaladdress"))
  ...
  const performValidation = () =>{
    ...
    if(name===''){
      Alert.alert('Fill in the your name');
    }else if(gender ===''){
      Alert.alert('Fill the gender');
    }else if(Age ===''){
      Alert.alert('Fill in your Age');
    }else if(Age<=25){
      Alert.alert('Age should be greater than 25');
    }else if(Age>=70){
      Alert.alert('Age should be less than 70');
    }else if(docregisterno ===''){
      Alert.alert('Fill in the Doctor regestration numebr');
    }else if(docregisterno.length<=6){
      Alert.alert('Registration number should be greater than 6 ');
    }else if(docregisterno.length>=15){
      Alert.alert('Registration number should be less than 15');
    }else if(specilization ===''){
      Alert.alert('Fill in the specilization');
    }else if(phonenumber ===''){
      Alert.alert('Fill in your Phone number');
    }else if(regmobNo.test(phonenumber) === false ){
      Alert.alert('Fill the Valid Phone Number');
    }else if(hospitalname ===''){
      Alert.alert('Fill in the hospital name');
    }else if(hospitaladdress ===''){
      Alert.alert('Fill in the hospital address');
    }else if(email ===''){
      Alert.alert('Fill in your Email');
  ...
  const submitData = () =>{
    fetch('http://'+global.IP+'/project/docreg.php', {
        method: 'POST',
        headers: {
        ...
        }else if(responseJson.message === "docregisterno exist"){
          alert("Doctor register number already exists");
        ...
        <ScrollView>
          <View style={styles.container}>
            <View style={styles.header}>
              <Text style={styles.textheader}>Doctor Registration!</Text>
            ...
            <View style={styles.footer}>
              ...
              <View style={styles.action}>
                <TextInput
                  style={styles.textinput}
                  placeholder='Doctors Regestration No'
                  placeholderTextColor='#8A8A8A'
                  value={docregisterno} 
                  keyboardType="number-pad"

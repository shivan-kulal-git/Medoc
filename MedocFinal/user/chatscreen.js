...
const DisplayEmployee = (props) => {
  ...
  const fetchData = () => {
    fetch('http://'+global.IP+'/project/userchatlist.php?Pid='+global.MyVar)
      .then(res => res.json())
      .then(result => {
        if (result.code === "404") {
  ...
  const ListEmptyView = () => {
    return (
      <View style={styles.MainContainer}>
        <Text style={{ textAlign: 'center' }}> Sorry, NO Consultation yet.</Text>
      </View>
    );
  }
              ...
              <TouchableOpacity>
                <Card onPress=
                  ...
                  <View style={styles.listItem}>
                    ...
                    <Text style={{ fontSize: 15, fontFamily: "sans-serif", left: '40%', }}>{item.Name}</Text>

...
const Ambulance = () => {
  ...
  const fetchData = () => {
    fetch('http://'+global.IP+'/project/Ambulance.php')
      .then(res => res.json())
      .then(result => {
        if (result.code === "404") {
              ...
              <View>
                <Card >
                  <View style={styles.listItem}>
                    <UserAvatar size={30} style={{ width: 60, height: 60, borderRadius: 30 }} name={item.Ambul_ID }></UserAvatar>
                    <Text style={{fontSize: 12, fontFamily: "sans-serif", left: '40%', }}>{item.Hospital_Name}</Text>
                    <Text style={{left:'40%',top:'80%',color: "green", fontSize: 12,  position: 'absolute' }}>{item.Hospital_Address}</Text> 
                    <Text style={{left:'40%',top:'110%',color: "green", fontSize: 12, position: 'absolute'  }}>{item.Phone_No}</Text> 
